            ...
            <div className='flex items-center max-lg:flex-col  w-full justify-between  gap-6   '>
                {brands.map((brand, index) => (
                    <div key={index} className='relative overflow-hidden group cursor-pointer'>
                        ...
                        <div className='absolute top-6 max-xl:top-8 max-lg:top-6 left-4'>
                            <Image className='max-xl:w-10 max-xl:h-10 max-lg:w-20 max-lg:h-20 ' src={brand.logo} alt="elitisbrand" />
                        </div>
                        <div className='absolute  left-24 max-xl:left-16 max-lg:left-28  space-y-2 xl:opacity-0 lg:group-hover:opacity-100 lg:top-[-100%] lg:group-hover:top-7 max-lg:top-7 duration-500 '>
                            <p className='text-xl max-xl:text-base  max-lg:text-3xl font-bold text-white max-lg:text-black '>{brand.name}</p>
                            <p className='text-white max-lg:text-black max-xl:text-xs max-lg:text-2xl text-sm'>{brand.place}</p>
                        </div>
                    </div>
                ))}
            </div>
