    ...
    return (
        <div className='desktop  max-md:w-[95%]  flex flex-col   gap-10 py-8  '>
            <div className='flex-col flex gap-2 text-left w-full max-lg:text-center'>
                ...
                <p className='text-base text-gray-400'>Lots of new products and product collections</p>
            </div>
            <div className='gap-6  w-full  grid grid-cols-5 max-md:grid-cols-2  '>
                {categories.map((category,index) => (
                    <div key={index} className='relative w-full group  overflow-hidden   '>
                        ...
                        <p className=' cursor-pointer  absolute xl:group-hover:top-32 lg:group-hover:top-12 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#EEEDE7]  text-black text-xl max-xl:text-sm max-md:text-xl rounded-3xl max-xl:px-3 max-md:px-8 px-8 py-1    duration-500'>{category.name}</p>
                        ...
                        <Image  className='  w-full' src={category.src} alt={category.name} style={{ objectFit: 'contain' }} />
                    </div>                        
                ))}                                
            </div>
        </div>
